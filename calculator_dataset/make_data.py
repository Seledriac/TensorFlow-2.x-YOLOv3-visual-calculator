...
add_path = "calculator_dataset"
if os.getcwd().split(os.sep)[-1] != "calculator_dataset":
    add_path = "calculator_dataset"
    os.chdir(add_path)
else:
    add_path = ""
...
def make_image(data, image_path, ratio=1):
    ...
    image = cv2.resize(image, (int(signs_size*ratio), int(signs_size*ratio)))
    h, w, c = image.shape
...
for file in ["train", "test"]:
    if not os.path.exists(f"dataset/{file}"):
        with ZipFile(f"dataset/{file}_calculator.zip", 'r') as zip:
...
for file in ['train','test']:
    images_path = os.getcwd()+f"/calculator_{file}"
    labels_txt = os.getcwd()+f"/calculator_{file}.txt"
    ...
    image_paths  = [os.path.join(os.path.realpath("."), os.getcwd()+f"/dataset/{file}/" + image_name)
                           for image_name in os.listdir(os.getcwd()+f"/dataset/{file}")]
